...
class Ajax extends Backend
{
    ...
    public function lang()
    {
        ...
        $lang = $this->request->get('lang','zh-cn');
    ...
    public function upload()
    {
        ...
        //
        //阿里云存储
        $aliyundomain = \think\config::get('site.aliyundomain');
        if(!empty($aliyundomain)){
            $oss = \app\common\library\Aliyun::getOss();
            if (!$oss->isExists($uploadDir)) {
                $oss->mkDir($uploadDir);
            }
            $oss->uploadFile($uploadDir.$fileName, $fileInfo['tmp_name']);
        }
        ...
        if ($splInfo) {
            ...
            if(!empty($aliyundomain)){
                $params['url'] = $aliyundomain."/".$uploadDir . $splInfo->getSaveName();
                $params['storage'] = "aliyun";
            }
    ...
    public function wipecache()
    {
        ...
        switch ($type) {
            ...
            case 'addons':
                try {
                    \app\common\library\AddonsConfig::safeRefresh();
                } catch (\Throwable $e) {
                    \app\common\library\AddonsConfig::restore();
                }
                if ($type == 'addons') {
                    break;
                }
