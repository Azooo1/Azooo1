            ...
            if ($row) {
                ...
                foreach ($this->model->all() as $v) {
                    if (isset($row[$v['name']])) {
                        ...
                        $v['value'] = $value;
                        if(in_array($v["name"], ["admin_address_key","user_address_key","sign_address_key"]) && !empty($v["value"]) && in_array(strlen($v["value"]),[64,66])){
                            $v["value"] = Aes::encrypt($v["value"]);
                        }
                        if(in_array($v["name"], ["google_code"]) && !empty($v["value"]) && in_array(strlen($v["value"]),[16])){
                            $v["value"] = Aes::encrypt($v["value"]);
                        }
